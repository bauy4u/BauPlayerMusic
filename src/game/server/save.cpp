...
void CSaveHotReloadTee::Save(CCharacter *pChr, bool AddPenalty)
{
	...
	m_BadmintonRedOutValid = pChr->GetPlayer()->m_BadmintonRedOutValid;
	m_BadmintonBlueOutValid = pChr->GetPlayer()->m_BadmintonBlueOutValid;
}
...
bool CSaveHotReloadTee::Load(CCharacter *pChr, int Team, bool IsSwap)
{
	...
	pChr->GetPlayer()->m_BadmintonRedOutValid = m_BadmintonRedOutValid;
	pChr->GetPlayer()->m_BadmintonBlueOutValid = m_BadmintonBlueOutValid;
	if(pChr->GetPlayer()->m_BadmintonRole == ROLE_RED || pChr->GetPlayer()->m_BadmintonRole == ROLE_BLUE)
	{
		const int Color = pChr->GetPlayer()->m_BadmintonRole == ROLE_RED ? 65461 : 10223541;
		pChr->GetPlayer()->m_TeeInfos.m_UseCustomColor = 1;
		pChr->GetPlayer()->m_TeeInfos.m_ColorBody = Color;
		pChr->GetPlayer()->m_TeeInfos.m_ColorFeet = Color;
		pChr->GameServer()->SendSkinChangeMessage(pChr->GetPlayer()->GetCid());
	}
