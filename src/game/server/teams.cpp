...
void CGameTeams::SendTeamsState(int ClientId)
{
	...
	{
		const int RainbowColor = m_pGameContext->RainbowName().GetColor(ClientId, i);
		const int DisplayTeam = RainbowColor == -1 ? m_Core.Team(i) : RainbowColor;
		Msg.AddInt(DisplayTeam);
		MsgLegacy.AddInt(DisplayTeam);
	}
