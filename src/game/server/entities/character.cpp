...
{
	// Gomoku consumes exactly one fire-input edge. Do not intercept every
	// input packet: OnDirectInput calls FireWeapon even while the button is held.
	if(CountInput(m_LatestPrevInput.m_Fire, m_LatestInput.m_Fire).m_Presses && GameServer()->TryGomokuPlaceFromFire(m_pPlayer->GetCid()))
		return;
	if(m_ReloadTimer != 0)
	{
	...
	const CTuningParams *pWeaponTuning = GameServer()->IsDuelParticipant(m_pPlayer->GetCid()) ? &CTuningParams::DEFAULT : GetTuning(m_TuneZone);
	...
	case WEAPON_HAMMER:
	{
		...
		CEntity *apEnts[MAX_CLIENTS];
		int Hits = GameServer()->HammerDice3D(m_pPlayer->GetCid(), ProjStartPos, m_Pos, TeamMask());
		...
		for(int i = 0; i < Num; ++i)
		{
			...
			float Strength = pWeaponTuning->m_HammerStrength;
		...
		if(Hits)
		{
			float FireDelay = pWeaponTuning->m_HammerHitFireDelay;
	...
	case WEAPON_GUN:
	{
		...
		{
			int Lifetime = (int)(Server()->TickSpeed() * pWeaponTuning->m_GunLifetime);
	...
	case WEAPON_SHOTGUN:
	{
		if(GameServer()->IsDuelParticipant(m_pPlayer->GetCid()))
		{
			// A duel's shotgun follows vanilla PvP/DM rules: five damaging
			// pellets. DDRace's normal shotgun is a single pulling laser.
			constexpr int ShotSpread = 2;
			constexpr float aSpreading[] = {-0.185f, -0.070f, 0.0f, 0.070f, 0.185f};
			for(int i = -ShotSpread; i <= ShotSpread; i++)
			{
				const float SpreadAngle = angle(Direction) + aSpreading[i + ShotSpread];
				const float CenterWeight = 1.0f - std::abs(i) / (float)ShotSpread;
				const float Speed = mix((float)pWeaponTuning->m_ShotgunSpeeddiff, 1.0f, CenterWeight);
				new CProjectile(
					GameWorld(), WEAPON_SHOTGUN, m_pPlayer->GetCid(), ProjStartPos, direction(SpreadAngle) * Speed,
					(int)(Server()->TickSpeed() * pWeaponTuning->m_ShotgunLifetime), false, false, -1, direction(SpreadAngle) * Speed);
			}
		}
		else
		{
			float LaserReach = pWeaponTuning->m_LaserReach;
	...
	case WEAPON_GRENADE:
	{
		int Lifetime = (int)(Server()->TickSpeed() * pWeaponTuning->m_GrenadeLifetime);
	...
	{
		float FireDelay;
		pWeaponTuning->Get(offsetof(CTuningParams, m_HammerFireDelay) / sizeof(CTuneParam) + m_Core.m_ActiveWeapon, &FireDelay);
		m_ReloadTimer = FireDelay * Server()->TickSpeed() / 1000;
	}
}
...
{
	m_Health = std::clamp(Health, 0, 10);
}
...
{
	if(GameServer()->HandleDuelDamage(this, Force, Dmg, From, Weapon))
		return true;
...
{
	if(!Remove && GameServer()->IsDuelParticipant(m_pPlayer->GetCid()) && !GameServer()->IsDuelWeaponAllowed(m_pPlayer->GetCid(), Weapon))
		return;
