	...
	CEntity(pGameWorld, CGameWorld::ENTTYPE_LASER)
{
	...
	m_DuelDefaultTuning = GameServer()->IsDuelParticipant(m_Owner);
...
bool CLaser::HitCharacter(vec2 From, vec2 To)
{
	...
	{
		float Strength;
		if(m_DuelDefaultTuning)
			Strength = CTuningParams::DEFAULT.m_ShotgunStrength;
		else if(!m_TuneZone)
	...
	else if(m_Type == WEAPON_LASER && !GameServer()->AreDuelOpponents(m_Owner, pHit->GetPlayer()->GetCid()))
	{
		pHit->UnFreeze();
	}
...
void CLaser::DoBounce()
{
	...
	if(Res)
	{
		if(!HitCharacter(m_Pos, To))
		{
			...
			else if(m_DuelDefaultTuning)
				m_Energy -= Distance + CTuningParams::DEFAULT.m_LaserBounceCost;
			else if(!m_TuneZone)
			{
			...
			int BounceNum = m_DuelDefaultTuning ? CTuningParams::DEFAULT.m_LaserBounceNum : Tuning()->m_LaserBounceNum;
			if(!m_DuelDefaultTuning && m_TuneZone)
...
void CLaser::Tick()
{
	...
	float Delay = m_DuelDefaultTuning ? CTuningParams::DEFAULT.m_LaserBounceDelay : (!m_TuneZone ? Tuning()->m_LaserBounceDelay : TuningList()[m_TuneZone].m_LaserBounceDelay);
