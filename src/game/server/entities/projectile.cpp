	...
	CEntity(pGameWorld, CGameWorld::ENTTYPE_PROJECTILE)
{
	...
	m_DuelVanillaShotgun = m_DuelDefaultTuning && m_Type == WEAPON_SHOTGUN;
...
vec2 CProjectile::GetPos(float Time)
{
	float Curvature = 0;
	float Speed = 0;
	const CTuningParams *pTuning = m_DuelDefaultTuning ? &CTuningParams::DEFAULT : (!m_TuneZone ? Tuning() : &TuningList()[m_TuneZone]);
...
void CProjectile::Snap(int SnappingClient)
{
	...
	if(NetworkClipped(SnappingClient, GetPos(Ct)))
		return;
	// A PvP shotgun is a vanilla projectile. Do not encode it as a DDRace
	// projectile or clients will apply the wrong visual/prediction path.
	if(m_DuelVanillaShotgun)
	{
