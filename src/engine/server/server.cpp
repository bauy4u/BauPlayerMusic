...
static void InitFakeClientAddress(CServer::CClient &Client, int ClientId)
{
...
int CServer::Init()
{
	for(auto &Client : m_aClients)
	{
		...
		Client.m_TurtleSoupDummy = false;
		Client.m_AuthKey = -1;
		Client.m_Latency = 0;
...
const NETADDR *CServer::ClientAddr(int ClientId) const
{
	...
	if(m_aClients[ClientId].m_TurtleSoupDummy)
	{
		return &m_aClients[ClientId].m_DebugDummyAddr;
	}
...
bool CServer::IsTurtleSoupDummy(int ClientId) const
{
	return ClientId >= 0 && ClientId < MAX_CLIENTS && m_aClients[ClientId].m_TurtleSoupDummy;
}
...
int CServer::NewClientNoAuthCallback(int ClientId, void *pUser)
{
	...
	pThis->m_aClients[ClientId].m_TurtleSoupDummy = false;
...
int CServer::DelClientCallback(int ClientId, const char *pReason, void *pUser)
{
	...
	pThis->m_aClients[ClientId].m_ShowIps = false;
	pThis->m_aClients[ClientId].m_DebugDummy = false;
	if(pThis->m_aClients[ClientId].m_TurtleSoupDummy)
		pThis->m_TurtleSoupDummyClientId = -1;
	pThis->m_aClients[ClientId].m_TurtleSoupDummy = false;
	pThis->m_aClients[ClientId].m_ForceHighBandwidthOnSpectate = false;
...
void CServer::UpdateDebugDummies(bool ForceDisconnect)
{
	...
	for(int DummyIndex = 0; DummyIndex < maximum(m_PreviousDebugDummies, g_Config.m_DbgDummies); ++DummyIndex)
	{
		...
		if(AddDummy && m_aClients[ClientId].m_State == CClient::STATE_EMPTY)
		{
			...
			InitFakeClientAddress(Client, ClientId);
...
void CServer::UpdateTurtleSoupDummy(bool ForceDisconnect)
{
	if(ForceDisconnect || !g_Config.m_SvTurtleSoupDummy)
	{
		if(m_TurtleSoupDummyClientId >= 0 && m_TurtleSoupDummyClientId < MAX_CLIENTS && m_aClients[m_TurtleSoupDummyClientId].m_TurtleSoupDummy)
			DelClientCallback(m_TurtleSoupDummyClientId, "Dropping Tee detective", this);
		m_TurtleSoupDummyClientId = -1;
		return;
	}

	if(m_TurtleSoupDummyClientId >= 0 && m_TurtleSoupDummyClientId < MaxClients() && m_aClients[m_TurtleSoupDummyClientId].m_TurtleSoupDummy)
	{
		CClient &Client = m_aClients[m_TurtleSoupDummyClientId];
		if(str_comp(Client.m_aName, g_Config.m_SvTurtleSoupDummyName) != 0)
			str_copy(Client.m_aName, g_Config.m_SvTurtleSoupDummyName, sizeof(Client.m_aName));
		CNetObj_PlayerInput Input = {0};
		Input.m_TargetX = 1;
		Input.m_TargetY = 0;
		Client.m_aInputs[0].m_GameTick = Tick() + 1;
	...
	m_TurtleSoupDummyClientId = -1;
	for(int ClientId = MaxClients() - 1; ClientId >= 0; --ClientId)
	{
		CClient &Client = m_aClients[ClientId];
		if(Client.m_State != CClient::STATE_EMPTY)
			continue;
